...
# Încărcăm produsele existente dacă fișierul există
...
def translate_text(text):
    if not text: return ""
    # Sortăm cheile după lungime descrescătoare pentru a evita suprascrierea parțială (ex: "Smart" înainte de "Smart Home")
...
def fetch_products(page=1, per_page=100):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Eroare HTTP {response.status_code} la pagina {page}")
            return None
    except Exception as e:
        print(f"Eroare de conexiune la pagina {page}: {e}")
        return None

print("Începere preluare produse prin WooCommerce API...")
page = 1
while True:
    ...
    for item in products_data:
        ...
        # Curățăm prețul (extragem primul număr găsit)
        import re
        ...
        # Formatăm imaginile
        formatted_images = [{"src": img.get('src')} for img in images if img.get('src')]
    ...
    print(f"Preluate {len(new_products)} produse noi până la pagina {page}. Total nou: {len(new_products)}")
    
    # Dacă am primit mai puține produse decât per_page, înseamnă că am ajuns la final
    if len(products_data) < 100:
        break
        
    page += 1
    time.sleep(1) # Pauză scurtă pentru a nu încărca serverul
